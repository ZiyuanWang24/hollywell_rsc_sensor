# Copyright (c) 2020 UW
# Author: Ziyuan Wang (wangzi@uw.edu)
#
# Based on the RSC pressure sensor (Honeywell) datasheet
...
class HRSC(object):
    ...
    def rsc_test(self, rate = 20):
        # self.set_speed(rate)
        totalTime = input('Total test time (s): ')
        interval = input('Time interval (s): ')

        times = int(totalTime)/int(interval)
        i = 0
        t = 0
        temp = []
        pressure = []
        ...
        axs[0].plot(dataTime, pressure, 'ro')
        axs[0].plot(dataTime, pressure, 'r')
        axs[0].set_title('Pressure (Psi)')
        # axs[0].set_xlabel('Time (s)')
        axs[0].set_ylabel('Pressure (Psi)')
        axs[1].plot(dataTime, temp, 'go')
        axs[1].plot(dataTime, temp, 'g')
